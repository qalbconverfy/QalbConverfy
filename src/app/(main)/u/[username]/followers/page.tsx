...
export default function FollowersPage() {
  ...
  const {
    ...
    queryFn: ({ pageParam }) => followService.getFollowers(username, pageParam),
    initialPageParam: 1,
    getNextPageParam: (lastPage, pages) =>
      lastPage.next ? pages.length + 1 : undefined,
  });

  if (isLoading) return <PageSpinner label="Loading followers..." />;
  if (isError) {
    return (
      <ErrorState
        description="Could not load followers."
        onRetry={() => refetch()}
      />
    );
  }
